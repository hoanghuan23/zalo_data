...
def upload_to_s3(file_path):
    try:
        ...
        s3.upload_file(Filename=file_path, Bucket=bucket_name, Key=s3_key, ExtraArgs={'ContentDisposition': 'inline', 'ContentType': content_type})
        imageS3Url=f"https://cdn.hellojob.jp/{s3_key}"
        return imageS3Url
    except Exception as e:
        print(f"Lỗi tải lên S3: {str(e)}")
def delete_file_s3(url):
    s3_key=url.replace("https://cdn.hellojob.jp/","")
    s3 = boto3.client('s3',
                      aws_access_key_id=aws_access_key,
                      aws_secret_access_key=aws_secret_key,
                      region_name=region_name)
...
if __name__ == "__main__":
    local_file_path = "C:/Users/ACER/Downloads/inan2.jpg"  
